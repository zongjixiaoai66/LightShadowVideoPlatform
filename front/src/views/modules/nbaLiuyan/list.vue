                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('nbaLiuyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/guangyingshipin/upload/nbaLiuyanMuBan.xls"
                        >批量导入NBA留言数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('nbaLiuyan','导入导出')"
                                ...
                                :on-success="nbaLiuyanUploadSuccess"
                                :on-error="nbaLiuyanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('nbaLiuyan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入NBA留言数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('nbaLiuyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "nbaLiuyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('nbaLiuyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">{{ contents.tableBtnFont == 1?'详情':'' }}</el-button>
                            <el-button v-if="isAuth('nbaLiuyan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}</el-button>
                            <el-button v-if="isAuth('nbaLiuyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'删除':'' }}</el-button>
    ...
    //当前表下拉框搜索条件
        ...
        methods: {
            ...
            getDataList() {
                ...
                params['nbaLiuyanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            nbaLiuyanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "nbaLiuyan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
